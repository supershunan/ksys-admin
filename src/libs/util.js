 ...
 * @returns {Array}
 */
export const getMenuByRouter = (list, userInfo) => {
  let res = []
  forEach(list, item => {
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      ...
      if ((hasChild(item) || (item.meta && item.meta.showAlways)) && showThisMenuEle(item, userInfo)) {
        obj.children = getMenuByRouter(item.children, userInfo)
      }
      if (item.meta && item.meta.href) obj.href = item.meta.href
      if (showThisMenuEle(item, userInfo)) res.push(obj)
    }
  })
  return res
}
