...
<script>
import { addPubData,infoData,updateData } from "@/api/user"
...
export default {
  ...
  data() {
    return {
        ...
        dataRules: {
            account: [
                { required: true, message: '请填写推广人账号', trigger: 'blur' }
            ],
            nickname: [
                { required: true, message: '请填写推广人昵称', trigger: 'blur' }
            ],
            status: [
  ...
  methods: {
    ...
    open(type,id) {
        ...
        this.dataForm = {}
        if (type == null) {
            this.modalTitle = '添加推广人员'
            this.modalType = 'add'
        } else {
            this.modalTitle = '编辑推广人员信息'
            this.modalType = type
            this.getInfo(id)
        }
    },
