    ...
    <Modal class="user-detail" v-model="modalShow" :title="modalTitle">
        <Form ref="dataForm" :model="dataForm" :label-width="labelWidth" :rules="dataRules">
            <Row :gutter="rowGutter">
                ...
                <Col span="16">
                    <FormItem prop="account" label="达人账号">
                        <Input type="text" v-model="dataForm.account" placeholder="请输入"></Input>
                    </FormItem>
                    <FormItem prop="nickname" label="达人昵称">
...
<script>
import { addExpertData,infoData,updateData } from "@/api/user"
...
export default {
  ...
  data() {
    return {
        ...
        dataRules: {
            account: [
                { required: true, message: '请填写达人账号', trigger: 'blur' }
            ],
            nickname: [
                { required: true, message: '请填写达人昵称', trigger: 'blur' }
            ],
            status: [
