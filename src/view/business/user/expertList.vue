            ...
            <Row :gutter="rowGutter">
                <Form ref="searchForm" :model="searchForm" :rules="searchRules" inline>
                    <FormItem prop="account">
                        <Input type="text" v-model="searchForm.account" placeholder="达人账号">
                            <Icon type="ios-color-filter" slot="prepend"></Icon>
                        </Input>
                    </FormItem>
                    <FormItem prop="nickname">
                        <Input type="text" v-model="searchForm.nickname" placeholder="达人昵称">
                            <Icon type="ios-color-filter" slot="prepend"></Icon>
                        </Input>
                    </FormItem>
                    <FormItem prop="phone">
                        <Input type="text" v-model="searchForm.phone" placeholder="达人电话">
                            <Icon type="ios-color-filter" slot="prepend"></Icon>
                        </Input>
                    </FormItem>
...
import detail from "./expertDetail.vue"
export default {
  name: 'expert-list',
  components: {
    detail
                ...
                render: (h, params) => {
                    ...
                    return h('div', [
                        h('Button', {
                            ...
                            style: {
                                marginRight: '5px',
                                display: this.authMap['expertUpdate']?'':'none'
                            },
                            on: {
                                click: () => {
                                    this.update(row)
                        ...
                        h('Button', {
                            ...
                            style: {
                                marginRight: '5px',
                                display: row.status == 1 && this.authMap['expertBan'] ? '' : 'none'
                            },
                            on: {
                                click: () => {
                                    this.ban(row)
                        ...
                        h('Button', {
                            ...
                            style: {
                                marginRight: '5px',
                                display: row.status == 2 && this.authMap['expertDel'] ? '' : 'none'
                            },
                            on: {
                                click: () => {
                                    this.ban(row)
