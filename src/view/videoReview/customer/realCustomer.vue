<template>
  <div :style="{height: isCustomerRoute ? '100%' : 0}">
    <UnreadMessageIndicator :count="totalUnreadCount" />
    <div class="chat-container" v-show="isCustomerRoute">
...
export default {
  ...
  watch: {
    '$route.path': {
      immediate: true,
      handler (newPath) {
        this.isCustomerRoute = newPath === '/customer'
      }
    },
  ...
  methods: {
    async handleNewMessage (message) {
      ...
      if (message.toUserId === this.currentUserInfo.id &&
    ...
    async sendMessage () {
      ...
      try {
        const message = {
          ...
          to: {
            id: this.selectedUser.fromUserId
            // avatar: this.currentUserInfo.avatar
          },
  ...
  async mounted () {
    ...
    await this.$store.dispatch('initWebSocket') // 初始化 WebSocket
